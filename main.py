...
class BuyModal(discord.ui.Modal, title="PayPayで購入"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not product.get("stock"):
            await interaction.followup.send("この商品はまだ商品ファイルが登録されていません。", ephemeral=True)
            return
...
class ProductView(discord.ui.View):
    def __init__(self, product_id: str):
        ...
        button = discord.ui.Button(
            ...
            style=discord.ButtonStyle.success if is_free else discord.ButtonStyle.primary,
    ...
    async def buy_callback(self, interaction: discord.Interaction):
        ...
        if not product.get("stock"):
            await interaction.response.send_message(
                "この商品はまだ商品ファイルが登録されていません。", ephemeral=True
            )
            return
        if not has_available_stock(product):
        ...
        try:
            ...
            print("Free delivery error:", repr(e))
            order["status"] = "delivery_failed"
            order["delivery_error"] = str(e)
            save_json(ORDERS_FILE, orders)
            restore_stock(product["id"], reserved)
            await interaction.followup.send(
                "❌ 商品のDM送信に失敗しました。管理者に連絡してください。", ephemeral=True
            )
            return
...
async def product_add(
    ...
    if stock < -1:
        await interaction.response.send_message(
            "在庫は `-1`（無限）または0以上で指定してください。", ephemeral=True
        )
        return
...
@admin_group.command(name="stock_set", description="商品の在庫数を変更")
@app_commands.describe(id="商品ID", amount="在庫数（-1で無限）")
async def stock_set(interaction: discord.Interaction, id: str, amount: int):
    if not await require_admin(interaction):
        return
    if amount < -1:
        await interaction.response.send_message(
...
async def panel(interaction: discord.Interaction, id: str):
    ...
    embed.add_field(name="在庫", value=stock_display(product), inline=True)
...
async def on_ready():
    ...
    print(f"Discord login: {bot.user}", flush=True)
...
def health():
    """
    Render用の自己修復ヘルスチェック。
    起動直後はDiscord接続に時間がかかる場合があるので180秒の猶予を設ける。
    Discord接続が長時間戻らない場合のみ503を返し、Render側の再起動対象にする。
    """
    now = time.monotonic()
    ...
    # 長時間Discordへ戻れない = Renderに異常として知らせる
    return jsonify({
        "ok": False,
        "discord_ready": False,
        "state": "unhealthy",
